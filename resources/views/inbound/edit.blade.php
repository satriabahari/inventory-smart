    ...
    <section class="p-8">
        <div class="container mx-auto">
            <div class="bg-white dark:bg-gray-800 p-8 rounded-lg shadow-md">
                ...
                <!-- Form -->
                <form action="{{ route('inbound.update', $data->id) }}" method="POST" class="space-y-4">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="grid grid-cols-[1fr_2fr] items-center">
                        ...
                        <select name="supplier_id" id="supplier_id" class="w-full border-gray-300 dark:bg-gray-700 dark:text-gray-50 bg-gray-100 rounded-md p-2" required>
                            @foreach ($suppliers as $supplier)
                                <option value="{{ $supplier->id }}" class="bg-gray-50 text-gray-500 dark:text-gray-300 hover:bg-gray-200" {{ $supplier->id == $data->supplier_id ? 'selected' : '' }}>{{ $supplier->name }}</option>
                            @endforeach
                        </select>
                    </div>
