{{-- <x-app-layout>
  <section class="h-full p-4 space-y-4">
    <div class="flex justify-between">
      <x-button route="customer.create" color="blue" text="Create" icon="fa-plus"/>
    </div>
    @if (count($datas) > 0)
          ...
          <tbody>
            ...
            <tr>
              ...
              <td class="text-neutral-50 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-50 py-1 px-4 flex space-x-4">
                <x-button :route="route('customer.edit', $data->id)" color="green" text="Edit" icon="fa-pen-to-square"/>
                <form action="{{route("customer.destroy", $data->id)}}" method="POST">
                  @csrf
                  @method("DELETE")
                  <x-button :route="route('customer.destroy', $data->id)" color="red" text="Delete" icon="fa-trash"/>
                </form>
              </td>
            </tr>
            @endforeach
