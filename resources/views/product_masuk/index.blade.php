<x-app-layout>
    <section class="h-full p-4 space-y-4">
        <div class="flex justify-between">
            <x-button :route="route('product_masuk.create')" color="blue" text="Create" icon="fa-plus"/>
        </div>
                    ...
                    <tbody>
                        @foreach ($data as $entry)
                            <tr>
                                ...
                                <td class="text-neutral-500 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-300 py-1 px-4 text-sm">{{ $entry->product->name }}</td>
                                <td class="text-neutral-500 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-300 py-1 px-4 text-sm">{{ $entry->supplier->name }}</td>
                                <td class="text-neutral-500 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-300 py-1 px-4 text-sm">{{ $entry->stock }}</td>
                                <td class="text-neutral-500 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-300 py-1 px-4 text-sm">{{ $entry->tanggal }}</td>
                                <td class="text-neutral-50 bg-neutral-50 dark:bg-gray-800 dark:text-neutral-50 py-1 px-4 flex space-x-4">
                                    <x-button :route="route('product_masuk.edit', $entry->id)" color="green" text="Edit" icon="fa-pen-to-square"/>
                                    <form action="{{ route('product_masuk.destroy', $entry->id) }}" method="POST">
                                        @csrf
                                        @method("DELETE")
                                        <button type="submit">
